...
class BagModel(nn.Module):
    ...
    def forward(self, input):
        ...
        #output = torch.empty((len(bags), len(NN_out[0])), device = device) # Original
        # TODO: IMPROVE THIS DIRTY CODE: allows size-altering aggregation functions (not very efficient):
        aggregate = self.aggregation_func(NN_out[inner_ids == bags[0]].squeeze(dim=3).squeeze(dim=2), dim=0)
        if type(aggregate).__name__ == "max":
           aggregate = aggregate.values

        output = torch.empty((len(bags), len(aggregate)), device=device)

        for i, bag in enumerate(bags):
            aggregate = self.aggregation_func(NN_out[inner_ids == bag].squeeze(dim=3).squeeze(dim=2), dim=0)
            if type(aggregate).__name__ == "max":
                aggregate = aggregate.values
            output[i] = aggregate
...
class MilDataset(Dataset):
    ...
    Args:
        ...
        transforms:
    '''
    def __init__(self, data, ids, labels, transforms=None):
        self.data = data
        self.labels = labels
        self.ids = ids
    ...
    def __getitem__(self, index):
        ...
        item = self.data[(self.ids[0] == self.bags[index]).tolist()]
        label = self.labels[index]

        # Item will have the path of the slices, we need to fetch them before transforming the data and returning it

        item_data = []
        for slice_path in item:
            # Load actual data and save it
            item_data.append(torchvision.io.read_image(slice_path).float() / 255)

        item = torchstack(item_data).to(label.device)

        if self.transforms:
            item = self.transforms(item)

        return item, self.ids[:, self.ids[0] == self.bags[index]], label
